...
def director_queryset(user=None):
...
def _director_field(queryset=None):

    return DirectorDinChoiceField(

        queryset=queryset if queryset is not None else director_queryset(),
...
class _DirectorAutocompleteMixin:
    ...
    def __init__(self, *args, user=None, **kwargs):
        ...
        self._dirkyc_user = user

        super().__init__(*args, **kwargs)

        if user is not None:

            self.fields["director"] = _director_field(director_queryset(user))
